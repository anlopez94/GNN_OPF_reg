...
class GNN(tf.keras.Model):
    def __init__(
        ...
        **kwargs,
        ...
        fnn_layers = self.create_ffn(self.hidden_units_readout, self.dropout_rate_readout)
    ...
    def call(self, input):
        ...
        if self.moving_edges:
            # print(input)
            input_tensor = input[0:self.num_nodes * self.num_features]
            input_tensor = tf.reshape(input_tensor, [self.num_nodes, self.num_features])
            edges = input[self.num_nodes * self.num_features:]
            edges = tf.reshape(edges, [self.edges.shape[0], self.edges.shape[1]])
